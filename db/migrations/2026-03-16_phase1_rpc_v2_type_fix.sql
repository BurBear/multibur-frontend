-- FASE 1 FIX
-- Corrige create_orden_con_detalles_v2 para usar los tipos reales de las columnas,
-- incluyendo columnas enum en ordenes y detalles_orden.
...
begin
  v_cliente_id := nullif(trim(coalesce(p_orden->>'cliente_id', '')), '');
  v_descripcion := nullif(trim(coalesce(p_orden->>'descripcion_trabajo', '')), '');
  v_fecha_entrega := nullif(trim(coalesce(p_orden->>'fecha_entrega', '')), '');
  v_prioridad := coalesce(nullif(trim(coalesce(p_orden->>'prioridad', '')), ''), 'NORMAL');
  v_estado := coalesce(nullif(trim(coalesce(p_orden->>'estado', '')), ''), 'DISENO');
  ...
  v_tiene_oc := coalesce(nullif(trim(coalesce(p_orden->>'tiene_oc', '')), ''), 'false');
  ...
  v_gramaje := nullif(trim(coalesce(p_detalles->>'gramaje', '')), '');
  v_medida_ancho := nullif(trim(coalesce(p_detalles->>'medida_ancho', '')), '');
  v_medida_alto := nullif(trim(coalesce(p_detalles->>'medida_alto', '')), '');
  v_material_id := nullif(trim(coalesce(p_detalles->>'material_id', '')), '');
  v_formato_id := nullif(trim(coalesce(p_detalles->>'formato_id', '')), '');
  v_cantidad := nullif(trim(coalesce(p_detalles->>'cantidad_solicitada', '')), '');
  v_demasia := nullif(trim(coalesce(p_detalles->>'demasia', '')), '');
  v_maquina_sugerida_id := nullif(trim(coalesce(p_detalles->>'maquina_sugerida_id', '')), '');
  v_tipo_impresion := nullif(trim(coalesce(p_detalles->>'tipo_impresion', '')), '');
  v_observacion_tecnica := nullif(trim(coalesce(p_detalles->>'observacion_tecnica', '')), '');
  v_requiere_juegos_placa := coalesce(nullif(trim(coalesce(p_detalles->>'requiere_juegos_placa', '')), ''), 'false');
  v_juegos_placa_total := nullif(trim(coalesce(p_detalles->>'juegos_placa_total', '')), '');
  v_juegos_placa_detalle := case
    when jsonb_typeof(p_detalles->'juegos_placa_detalle') = 'array' then p_detalles->'juegos_placa_detalle'
    else null
  end;
  v_color_mode := nullif(trim(coalesce(p_detalles->>'color_mode', '')), '');
  v_color_text := nullif(trim(coalesce(p_detalles->>'color_text', '')), '');
  v_corte := coalesce(nullif(trim(coalesce(p_detalles->>'corte', '')), ''), 'false');
  v_empaquetado := coalesce(nullif(trim(coalesce(p_detalles->>'empaquetado', '')), ''), 'false');
  v_doblez := coalesce(nullif(trim(coalesce(p_detalles->>'doblez', '')), ''), 'false');
  v_compaginado := coalesce(nullif(trim(coalesce(p_detalles->>'compaginado', '')), ''), 'false');
  v_troquelado := coalesce(nullif(trim(coalesce(p_detalles->>'troquelado', '')), ''), 'false');
  v_sectorizado := coalesce(nullif(trim(coalesce(p_detalles->>'sectorizado', '')), ''), 'false');
  v_barniz := coalesce(nullif(trim(coalesce(p_detalles->>'barniz', '')), ''), 'false');
  v_plastificado := nullif(trim(coalesce(p_detalles->>'plastificado', '')), '');
